...
#-------------------------
ibp_plants = sf::read_sf('data/gis/ibp_plant_outlines.geojson')

band_names = c('b1','b2','b3','b4','b5')
ibp = raster::brick('data/imagery/uav_true_color/GIBPE_Aug12_2020.tif')
...
raster::writeRaster(ibp_predicted, 'data/ibp_predicted_cover.tif')

# nort
nort = raster::brick('data/imagery/uav_true_color/NORT_Aug14_2019.tif')
nort = raster::aggregate(nort, fact=5)
names(nort) <- band_names

nort_predicted = raster::predict(nort, cover_model)
raster::writeRaster(nort_predicted, 'data/nort_predicted_cover.tif')

# p9
p9 = raster::brick('data/imagery/uav_true_color/P9_Jul30_2019.tif')
p9 = raster::aggregate(p9, fact=5)
names(p9) <- band_names

p9_predicted = raster::predict(p9, cover_model)
raster::writeRaster(p9_predicted, 'data/p9_predicted_cover.tif')
